...
TkEntry.new(root, textvariable: nombre_matriz, font: 'arial 14', justify: 'right').pack(pady: 10, padx: 10, fill: 'x')
  ...
  fila.each do |btn_text|
    TkButton.new(boton_fila) do
      ...
      command do
        ...
        when 'Almacenar'
          nombre = nombre_matriz.value
          if nombre.empty?
            entrada.value = "Error: Proporcione un nombre para la matriz"
          else
            parser = RecursiveDescentParser.new(entrada.value)
            begin
              matriz = parser.parse
              almacenar_matriz(nombre, matriz)
              entrada.value = "Matriz almacenada como '#{nombre}'"
            rescue StandardError => e
              entrada.value = "Error: #{e.message}"
            end
          end
        when 'Recuperar'
          ...
          if nombre.empty?
            ...
            matriz_recuperada = obtener_matriz(nombre)
            if matriz_recuperada
              entrada.value = matriz_recuperada.to_s
            else
              entrada.value = "Error: Matriz '#{nombre}' no encontrada"
            end
          end
        when 'Salir'
          exit
